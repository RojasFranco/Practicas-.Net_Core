...
namespace Curso.Api.Controllers
{
	...
	public class SecurityController : ControllerBase
    {
		...
		public async Task<ActionResult> Login([FromBody] UserDTO user)
		{
			...
			else return Ok(new ResultJson() { Message = dbUser.DefaultPage, TokenActualizado = token });
		}
		...
		public async Task<ActionResult> ValidarUsuarioPorToken(string token)
		{			
			
			if(!Guid.TryParse(token, out Guid guidValidado))
			{
				return BadRequest(new ResultJson() { Message = "el token no es valido" });
			}

			var retorno = await _validacionTiempoEnLinea.VerificarTiempo(guidValidado);
			if(retorno)
			{
				return Ok(new ResultJson() { Message = "Esta activo" });
			}
			else
			{
				return Unauthorized(new ResultJson() { Message = "Paso mas de 10 min de conexion, logueese nuevamente" });
			}
		}
