$(document).ready(function () {
	...
	$("#btnVerificarConexion").click(function () {
		$("#Inicial").hide();
		$("#TokenValidar").show();
	...
	$("#btnVolverDeVerificar").click(function () {
		$("#TokenValidar").hide();
		$("#Inicial").show();
	});
	...
	$("#btnVerificarToken").click(function () {		
		var token = $("#txtTokenValidar").val();
		if (token != "") {

			$.ajax({
				url: "https://localhost:5003/api/Autorizacion/Validacion/" + token,				
				crossDomain: true,
				dataType: 'json',
				headers: {
	...
	$("#guardarPersona").click(function () {
		...
		if (!isNaN(parseInt(dniNew)) && nombreNew.length > 2 && apellidoNew.length > 2) {
			...
			$.ajax({
				url: "https://localhost:5003/api/Tables/CrearPersona/" + token,
...
//******************************** BAJA PERSONA	**************************************************
	$("#btnEliminar").click(function () {
		...
		if (dniEliminar.length > 0 && /^[0-9]*$/.test(dniEliminar)) {
			...
			let token = localStorage.getItem("TOKEN");
			$.ajax({
				url: "https://localhost:5003/api/Tables/BorrarPersona/" + userEliminar + "/" + token,
				method: "DELETE",
